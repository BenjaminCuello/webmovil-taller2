...
// Si no funciona en tu emulador, puedes sobreescribir el host guardando en localStorage:
//   localStorage.setItem('API_HOST', '192.168.x.y')  // IP LAN de tu PC
// Luego reinicia la app. Esto permite usar la misma build en emulador o dispositivo físico
...
(function () {
	...
	try {
		// Si corremos en navegador (no Cordova), usa localhost por comodidad
	...
	// URLs efectivas
	var BASE_URL_POKEMON = make(3000)
	var BASE_URL_COUNTRIES = make(4000)
	var BASE_URL_FASTAPI = make(8000)

	// Exponer como globales para el resto del código
	...
	// Log auxiliar en caso de tener el debug overlay
	if (typeof window.debugLog === 'function') {
		window.debugLog('config.js', {
