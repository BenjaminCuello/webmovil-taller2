...
API NestJS que expone información de Pokémon almacenada en PostgreSQL y la sirve con un contrato similar a la PokeAPI, pero usando sprites locales (`/img/pokemon/...`).
...
La API se construye como servicio `nest` dentro del `docker-compose.yml` de la raíz.
...
1. Configura las variables de entorno para la conexión a PostgreSQL (las mismas que en `docker-compose.yml`):
...
- `GET /pokemon?limit=&offset=` devuelve una lista paginada (por defecto `limit=20`, `offset=0`, `limit` máximo 50).
- `GET /pokemon/:idOrName` busca por `id` numérico o por nombre (case-insensitive).
...
Los sprites referenciados (`/img/pokemon/...`) se sirven desde el frontend (`frontend/www/img/pokemon/`) y permiten que las imágenes funcionen sin conexión a Internet.
